...
同时设计了python测试脚本，随机生成模块的输入数据，调用modelsim进行仿真，并将仿真结构与golden model计算结果比较，以验证硬件实现的正确性。进行测试时请根据`README`的步骤进行。
...
本设计为三级流水线结构，数据输入后经过三个时间周期得到对应的结果，因此计算延迟为三个时钟周期。

流水线启动后，每周期输入一组QKV输出进行处理，吞吐率为3*8*4*16bit=#(3*8*4*16)bit每周期。考虑到启动延迟，假设执行N次注意力计算操作，吞吐率$N/(N+3)$
